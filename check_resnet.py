...
if not FLAGS.is_parsed():
    FLAGS(['test'])  # Dummy argv to trigger parsing

from tf2.resnet import resnet as tf_resnet
from pytorch.resnet import resnet as torch_resnet

# Model configuration
RESNET_DEPTH = 50
WIDTH_MULTIPLIER = 1
CIFAR_STEM = False
DROPBLOCK_PROBS = [None, None, None, None]
DROPBLOCK_SIZE = None
...
tf_model = tf_resnet(
    ...
    dropblock_keep_probs=DROPBLOCK_PROBS,
    dropblock_size=DROPBLOCK_SIZE
)

# PyTorch model
torch_model = torch_resnet(
    resnet_depth=RESNET_DEPTH,
    width_multiplier=WIDTH_MULTIPLIER,
    cifar_stem=CIFAR_STEM,
    data_format='channels_first',  # PyTorch uses NCHW
    dropblock_keep_probs=DROPBLOCK_PROBS,
    dropblock_size=DROPBLOCK_SIZE
)
...
input_torch = torch.tensor(np.transpose(input_np, (0, 3, 1, 2)), dtype=torch.float32)

# ====== Inference ======
print("Running inference...")

with torch.no_grad():
    output_torch = torch_model(input_torch).cpu().numpy()

output_tf = tf_model(input_tf, training=False).numpy()

# ====== Compare ======
print("TF output shape:    ", output_tf.shape)
print("PyTorch output shape:", output_torch.shape)

if output_tf.shape != output_torch.shape:
    print("Mismatch in output shape. Check final pooling/data_format.")
    exit(1)

# Numerical comparison
close = np.allclose(output_tf, output_torch, rtol=1e-4, atol=1e-4)
max_diff = np.max(np.abs(output_tf - output_torch))

print("Outputs are numerically close:", close)
print("Max difference:", max_diff)

if not close:
    print("Sample TF output:   ", output_tf.flatten()[:10])
    print("Sample PyTorch output:", output_torch.flatten()[:10])
...
print(f'Torch Total parameters: {num_params:,}')

num_trainable_params = sum(p.numel() for p in torch_model.parameters() if p.requires_grad)
print(f'Torch Trainable parameters: {num_trainable_params:,}')
